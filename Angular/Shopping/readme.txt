...
3. Firefox will smoothly run these HTMLs if index.html file is opened directly.

4. Chrome won't run these HTMLs if index.html file is opened directly because app uses HTTP call for rendering partials and getting JSON stubs. Chrome doesn't allow local file system calls and hence these HTMLs won't run. These HTMLs need to be put on a Web Server to run on Chrome.

5. I have written basic Jasmine test cases located at "resources/js/app/tests/". Karma config file is located at root folder.